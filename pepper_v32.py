...
# -*- coding: utf-8 -*-
"""
PEPPER CLINICAL INFINITY v3.2
Commander: Lamya | Omdurman Islamic University

FIXES v3.2:
  ✅ TTS: runs in dedicated daemon thread, no pyttsx3/Qt conflict
  ✅ Emotion window: exactly 300x300, always on top, working detection
  ✅ Live session: independent frame buffer, no freeze
  ✅ Speech queue: only exact keyword match counts as fail
  ✅ Tablet touch: cards never falsely locked
  ✅ Full-body detection: sensitive thresholds for children
  ✅ 10-finger count both hands
  ✅ 2 wrong → CSV report + auto-advance
  ✅ Endless ESDM/ABA/TEACCH/DTT tasks
  ✅ Animated Pepper avatar W3 + Tablet
  ✅ Text chat + YouTube how-to links
...
os.environ.update({
    ...
    'TF_ENABLE_ONEDNN_OPTS':'0',  'PULSE_LATENCY_MSEC':'30',
    'MPLBACKEND':'Agg', 'GDK_BACKEND':'x11',
})
...
# ── Child setup ─────────────────────────────────────────────────────
print("\n" + "═"*56)
print("  PEPPER CLINICAL INFINITY v3.2")
...
_MTR=[
    ...
    {"id":"wave","name":"👋 Wave","verify":"wave","protocol":"ESDM",
     ...
     "waiting":"Wave your hand! 👋","success":"Great wave! ✅","fail":"Wave side to side!"},
    ...
    {"id":"arms_out","name":"🤸 Arms Wide","verify":"arms_out","protocol":"ESDM",
     "instruction":"Stretch BOTH arms WIDE! 🤸",
    ...
    {"id":"jump","name":"⬆️ Jump","verify":"body_motion","protocol":"ESDM",
     "instruction":"JUMP up! Jump jump! ⬆️",
    ...
    {"id":"spin","name":"🌀 Spin","verify":"body_motion","protocol":"TEACCH",
     "instruction":"SPIN in a circle! 🌀",
     "waiting":"Spin spin! 🌀","success":"Great spinning! ✅","fail":"Turn your whole body!"},
    {"id":"march","name":"🪖 March","verify":"body_motion","protocol":"ESDM",
     "instruction":"MARCH! Left right! 🪖",
...
def build_pool(n=5000):
    ...
    for _ in range(int(n*.04)):
        ...
        opts=[t]+d; random.shuffle(opts)
        cor=next(i for i,o in enumerate(opts) if o["id"]==t["id"])
        P.append({"id":f"vehicle_{t['id']}","domain":"Cognitive",
                  "protocol":"DTT","level":3,
                  "name":f"🚗 Find {t['label']}",
                  "instruction":f"Touch the {t['label']}!",
                  "waiting":f"Find the {t['label']}! 🚗",
...
ST = {
    ...
    "last_speech":"","last_sound":time.time(),
    "heard_flag":False,"recording":False,"listening":True,
    # tts — uses a queue so pyttsx3 runs in its own thread
    "tts_q": queue.Queue(),
    ...
    "task_history":[],
    "emo_timeline":[],
    # latest camera frame (shared between threads, no cap sharing)
    "latest_frame": None,
...
# ── TTS Thread ───────────────────────────────────────────────────────
# FIX: pyttsx3 must run in its OWN thread, never called from Qt thread
class TTSThread(threading.Thread):
    """Dedicated TTS daemon. Reads from ST['tts_q'] and speaks."""
    def __init__(self):
        super().__init__(daemon=True, name="TTS-Thread")
        self.engine = None
    ...
    def run(self):
        ...
        while True:
            try:
                text = ST["tts_q"].get(timeout=0.2)
                if text is None: break        # stop signal
                if not self.engine: continue
                ST["is_speaking"] = True
                # Lip sync in parallel
                threading.Thread(target=self._lip,
                                 args=(text,), daemon=True).start()
                try:
                    self.engine.say(text)
                    self.engine.runAndWait()
                except Exception as e:
                    LOG(f"TTS speak err: {e}", "warn")
                ST["is_speaking"] = False
                ST["lip"] = 0.0
            except queue.Empty:
                pass
            except Exception as e:
                LOG(f"TTS loop err: {e}", "warn")
                ST["is_speaking"] = False
                ST["lip"] = 0.0
    ...
    def say(self, text):
        """Queue text for speaking — safe to call from any thread."""
...
TTS = TTSThread()
TTS.start()
TTS._ready.wait(timeout=3.0)

# ── Camera capture thread ────────────────────────────────────────────
# Single thread owns the cap. Writes frames to ST["latest_frame"].
class CaptureThread(threading.Thread):
    """Owns VideoCapture. Writes frames into shared state."""
    def __init__(self):
...
# ── MediaPipe thread ─────────────────────────────────────────────────
# Reads from capture, does pose/hands/facemesh, updates ST
class MPThread(threading.Thread):
    ...
    def _init_mp(self):
        ...
        try:
            self._hands = mp.solutions.hands.Hands(
                ...
                min_detection_confidence=0.35,
                min_tracking_confidence=0.35)
            print("✅ MP Hands 10-finger (sens=0.35)")
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self._pose:
                try:
                    ...
                    if res.pose_landmarks:
                        ...
                        ST["face_lm"].update({
                            "nose_x":pl["nx"]*w2,"nose_y":pl["ny"]*h2,
                            "fw":w2,"fh":h2})
                    else:
            ...
            if self._face:
                try:
                    ...
                    if fr.multi_face_landmarks:
                        ...
                        ST["blink_v"] = ea
                        # Emotion from FaceMesh
                        scores = {"neutral":0.4}
                        if ea < 0.19:
                            scores = {"surprised":0.85,"neutral":0.15}
                        elif mo > 0.07:
                            scores = {"happy":0.88,"neutral":0.12}
                        elif mo > 0.045:
                            scores = {"happy":0.60,"joyful":0.40}
            ...
            self._validate()
            time.sleep(0.025)  # ~40fps analysis

    def stop(self): self.running = False

# ── Emotion window thread ────────────────────────────────────────────
# Completely independent OpenCV window — EXACTLY 300×300 pixels
class EmotionWin(threading.Thread):
    WIN = "Emotions"  # short title fits window
    COLS = {
    ...
    EMJS = {
        ...
        "angry":"ANGRY >:(","fear":"SCARED :(","surprised":"WOW :O",
        "disgust":"DISGUSTED","neutral":"NEUTRAL :|","excited":"EXCITED!!",
    }
    ...
    def run(self):
        W = H = 300
        # WINDOW_AUTOSIZE means the window size = image size, NOT resizable
        ...
        while self.running:
            ...
            # Update face crop every 3 frames
            if self._fc % 3 == 0:
                frame = self.cap.get_frame()
                h, w = frame.shape[:2]
                fm = ST.get("face_lm", {})
                ...
                if nx > 0 and ny > 0:
                    ...
                    x2 = min(w, x1+fs)
                    y2 = min(h, y1+int(fs*1.1))
                    if x2 > x1+10 and y2 > y1+10:
                        self._face_crop = frame[y1:y2, x1:x2].copy()

            # Build exactly 300×300 panel
    ...
    def _build(self, W, H):
        ...
        if fc is not None and fc.size > 0:
            try:
                ...
                cv2.rectangle(canvas, (0,0), (W-1, face_h-1), col, 2)
            except: pass
        else:
            canvas[0:face_h, :] = (18, 22, 48)
            cv2.putText(canvas, "NO FACE", (90, 66),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (60,60,90), 2)

        # ── Emotion label ───────────────────────────────
        y_label = face_h + 2
        cv2.rectangle(canvas, (0, y_label), (W, y_label+28), (0,0,0), -1)
        lbl = self.EMJS.get(em, em.upper())
        cv2.putText(canvas, lbl, (6, y_label+20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.60, col, 2)
        # face dot
        dc = (0,200,0) if ST["face_detected"] else (0,0,180)
        cv2.circle(canvas, (W-14, y_label+14), 8, dc, -1)

        # ── Bars: 8 emotions, fill remaining height ──────
        emos = ["happy","sad","angry","surprised",
                "fear","disgust","neutral","excited"]
        bars_top = y_label + 30
        bars_h   = H - bars_top - 2
        bar_h    = max(14, bars_h // len(emos))
        ...
        for i, e in enumerate(emos):
            ...
            # background
            cv2.rectangle(canvas, (0,y), (W, y+bar_h-1), (16,18,34), -1)
            # fill proportional to score
            fw = int(sc * W)
            if fw > 0:
                cv2.rectangle(canvas, (0,y), (fw, y+bar_h-1), ec, -1)
            # border — thicker if active
            brd = 2 if act else 1
            brc = ec if act else (35,38,58)
            cv2.rectangle(canvas, (0,y), (W, y+bar_h-1), brc, brd)
            # label
            txt = f"{e[:7].upper()}: {sc:.0%}"
            cv2.putText(canvas, txt, (4, y+bar_h-3),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.30,
                        (255,255,255) if act else (120,120,120),
                        2 if act else 1)

        return canvas   # exactly 300×300

    def stop(self): self.running = False

# ── Avatar draw helper (OpenCV) ──────────────────────────────────────
def draw_av(cv2_img, cx, cy, ph, lip, spk, lst, joy, blink, tilt, scale=1.0):
    s=scale
    hbob=int(4*math.sin(ph*0.5)*s); ht=int(tilt*14*s)
    hy=cy-int(54*s)+hbob; hx=cx+ht
    # Torso
    cv2.ellipse(cv2_img,(cx,cy+int(82*s)),(int(48*s),int(70*s)),0,0,360,(70,90,190),-1)
    cv2.ellipse(cv2_img,(cx,cy+int(82*s)),(int(48*s),int(70*s)),0,0,360,(100,120,220),2)
    # Chest light
    cl=(0,220,200) if spk else (0,100,200)
    cv2.circle(cv2_img,(cx,cy+int(58*s)),int(9*s),cl,-1)
    cv2.circle(cv2_img,(cx,cy+int(58*s)),int(11*s),(0,180,255),1)
    # Arms
    if joy:
        jt=time.time()
        la=int(30*math.sin(jt*4)*s); ra=int(30*math.sin(jt*4+1)*s)
        cv2.ellipse(cv2_img,(cx-int(58*s)+la,cy+int(28*s)),(int(12*s),int(52*s)),-62+la,0,360,(70,90,190),-1)
        cv2.ellipse(cv2_img,(cx+int(58*s)+ra,cy+int(28*s)),(int(12*s),int(52*s)),62+ra,0,360,(70,90,190),-1)
    elif spk:
        la=int(22*math.sin(ph)*s); ra=int(22*math.sin(ph+math.pi)*s)
        cv2.ellipse(cv2_img,(cx-int(58*s)+la,cy+int(62*s)),(int(11*s),int(36*s)),-30+la,0,360,(70,90,190),-1)
        cv2.ellipse(cv2_img,(cx+int(58*s)+ra,cy+int(62*s)),(int(11*s),int(36*s)),30+ra,0,360,(70,90,190),-1)
    else:
        cv2.ellipse(cv2_img,(cx-int(58*s),cy+int(68*s)),(int(11*s),int(32*s)),-15,0,360,(70,90,190),-1)
        cv2.ellipse(cv2_img,(cx+int(58*s),cy+int(68*s)),(int(11*s),int(32*s)),15,0,360,(70,90,190),-1)
    # Legs
    cv2.rectangle(cv2_img,(cx-int(28*s),cy+int(148*s)),(cx-int(10*s),cy+int(180*s)),(55,75,165),-1)
    cv2.rectangle(cv2_img,(cx+int(10*s),cy+int(148*s)),(cx+int(28*s),cy+int(180*s)),(55,75,165),-1)
    # Head
    cv2.circle(cv2_img,(hx,hy),int(53*s),(220,195,173),-1)
    cv2.circle(cv2_img,(hx,hy),int(53*s),(200,175,155),2)
    # Ears
    for ex in [hx-int(52*s),hx+int(52*s)]:
        cv2.circle(cv2_img,(ex,hy-int(5*s)),int(10*s),(210,185,163),-1)
        cv2.circle(cv2_img,(ex,hy-int(5*s)),int(7*s),(240,200,180),-1)
    # Eyes
    eyh=max(1,int(7*s)) if not blink else 1
    ec=tuple(int(x) for x in ST["eye_color"])
    for ex in [hx-int(19*s),hx+int(19*s)]:
        cv2.ellipse(cv2_img,(ex,hy-int(9*s)),(int(7*s),eyh),0,0,360,ec,-1)
        if not blink:
            cv2.circle(cv2_img,(ex+1,hy-int(9*s)),int(3*s),(255,255,255),-1)
            cv2.circle(cv2_img,(ex+2,hy-int(10*s)),int(1*s),(0,0,0),-1)
    # Nose
    nt=int(ST.get("nose_tw",0)*0.5*s)
    cv2.circle(cv2_img,(hx+nt,hy+int(7*s)),int(5*s),(180,140,120),-1)
    # Mouth
    mh=int((4+lip*20)*s)
    if spk:
        cv2.ellipse(cv2_img,(hx,hy+int(22*s)),(int(15*s),max(2,mh)),0,0,180,(160,80,80),-1)
        cv2.ellipse(cv2_img,(hx,hy+int(22*s)),(int(15*s),max(2,mh)),0,0,180,(210,110,110),2)
    elif ST["emotion"] in ["happy","joyful","excited"]:
        cv2.ellipse(cv2_img,(hx,hy+int(20*s)),(int(14*s),int(7*s)),0,0,180,(150,80,80),-1)
    else:
        cv2.line(cv2_img,(hx-int(12*s),hy+int(21*s)),(hx+int(12*s),hy+int(21*s)),(150,80,80),2)
    # Ring
    if joy:
        jt=time.time()
        pr=int((84+14*abs(math.sin(jt*4)))*s)
        jc=(int(128+127*math.sin(jt*3)),int(200+55*math.sin(jt*2)),255)
        cv2.circle(cv2_img,(cx,cy),pr,jc,3)
    elif spk:
        pr=int((88+5*math.sin(ph*5))*s)
        cv2.circle(cv2_img,(cx,cy),pr,(0,160,255),2)
    elif lst:
        cv2.circle(cv2_img,(cx,cy),int(90*s),(0,220,100),2)
    return cv2_img

# ── Live window W3 ───────────────────────────────────────────────────
# Reads from capture thread, NO direct cap sharing → no freeze
class LiveWin(threading.Thread):
    ...
    def run(self):
        ...
        cv2.moveWindow(self.WIN, 740, 20)
        ...
        while self.running:
            ...
            time.sleep(0.033)   # ~30fps — comfortable for display
    ...
    def _build(self, frame):
        ...
        cv2.putText(win,
            f"PEPPER v3.2 — {CHILD_NAME} — ABA/ESDM/TEACCH/DTT",
...
class Tablet(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Pepper Tablet v3.2 — {CHILD_NAME}")
        ...
        self._joy_ph=0.0; self._capture=None
        self._joy_t=QTimer(); self._joy_t.timeout.connect(self._joy_tick)
        self._speech=None; self._gemini=None
        self._build_ui()
        self._connect()
        self._st=QTimer(); self._st.timeout.connect(self._stats)
        self._st.start(350)
        # Camera update timer — reads from shared frame buffer
        self._cam_t=QTimer(); self._cam_t.timeout.connect(self._update_cam)
        self._cam_t.start(33)   # ~30fps

    def set_capture(self, c): self._capture = c
    ...
    def _build_ui(self):
        ...
        self.av=AvWidget(185,215); frow.addWidget(self.av)
        ll.addLayout(frow)
        ...
        self.title_lbl=QLabel("PEPPER CLINICAL INFINITY v3.2")
        ...
        self.cl=QVBoxLayout(self.cf); self.cl.setContentsMargins(12,10,12,10)
        ...
        for lbl,attr,col in [
            ("Score","s_score","#a78bfa"),("Tokens","s_tok","#fbbf24"),
            ("Mastered","s_mast","#34d399"),("Streak","s_str","#60a5fa"),
            ("Wrong","s_wrg","#f87171"),("Auto","s_auto","#fb923c"),
        ]:
    ...
    # ── Camera update (Qt timer → safe) ─────────────────────
    def _update_cam(self):
        if self._capture is None: return
        frame = self._capture.get_frame()
        # Draw skeleton overlay from pose landmarks
        frame = self._draw_hud(frame.copy())
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        h_, w_, ch = rgb.shape
    ...
    def _stats(self):
        for attr,val in [
            ("s_score",str(ST["score"])),("s_tok",str(ST["tokens"])),
            ("s_mast",str(ST["tasks_mastered"])),("s_str",str(ST["streak"])),
            ("s_wrg",str(ST["wrong_count"])),("s_auto",str(ST.get("tasks_auto",0))),
        ]:
    ...
    def _on_suc(self, msg):
        ...
        ml.setAlignment(Qt.AlignmentFlag.AlignCenter); ml.setWordWrap(True)
        ol.addWidget(ml); self.cl.addWidget(ov)
    ...
    def _transcribe(self, audio) -> str:
        if self.whisper:
            try:
                ...
                if text:
                    LOG(f"Whisper: {text}"); return text.lower()
            except Exception as e:
            ...
            try:
                with sr.Microphone() as src:
                    ...
                    try:
                        ...
                        ST["recording"]=False; ST["listening"]=True
            except Exception as e:
                time.sleep(0.5)
            time.sleep(0.04)
...
# ── Gemini ────────────────────────────────────────────────────────────
EMPATHY={
    "ok":["PERFECT! Amazing! [CLAP]","WOW! [DANCE]","BRILLIANT! [CELEBRATE]"],
    "retry":["Not quite! Try again! 🎯","Almost! One more try!","You can do it!"],
    "up":["LEVEL UP! [DANCE]","CHAMPION! [CELEBRATE]"],
    "timeout":["No problem! Next task! 🎯","Moving on! Great effort!"],
    "wrong2":["Let us try a new task! Good try!","Moving on! Well done trying!"],
...
class Gemini:
    ...
    def ask(self, prompt):
        ...
        ctx=(f"[child={CHILD_NAME} level={ST['level']} dom={ST['domain']} "
...
@api_app.route("/")
def _api_root():
    return jsonify({"service":"Pepper API v3.2","child":CHILD_NAME})
...
.hdr{background:linear-gradient(135deg,#1a0a3d,#0a0f28);
  padding:16px 24px;border-bottom:2px solid #4f46e5;
  display:flex;justify-content:space-between;align-items:center}
.hdr h1{font-size:1.3em;color:#a78bfa}
.hdr .sub{color:#60a5fa;font-size:.83em}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;padding:16px}
.card{background:#0c0f1e;border:1px solid #1a1f40;border-radius:14px;padding:15px}
.card h3{font-size:.82em;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:9px}
.num{font-size:2.2em;font-weight:bold}
.g{color:#34d399}.b{color:#60a5fa}.y{color:#fbbf24}.r{color:#f87171}.p{color:#a78bfa}
.ebar{height:22px;border-radius:4px;margin:2px 0;position:relative;overflow:hidden}
.ebar .fill{position:absolute;left:0;top:0;bottom:0;border-radius:4px}
.ebar .lbl{position:relative;z-index:1;font-size:.68em;font-weight:bold;padding:4px 6px}
.chat-p{background:#1e1b4b;color:#b0a0ff;padding:5px 8px;border-radius:6px;margin:2px 0;font-size:.78em}
.chat-c{background:#0a2818;color:#80e880;padding:5px 8px;border-radius:6px;margin:2px 0;font-size:.78em}
.ring{width:82px;height:82px;border-radius:50%;display:flex;align-items:center;
  justify-content:center;font-size:1.5em;font-weight:bold;border:4px solid #34d399}
.rfbtn{background:#4f46e5;color:white;border:none;border-radius:8px;
  padding:6px 14px;cursor:pointer;font-size:.80em}
.rfbtn:hover{background:#6366f1}
.span2{grid-column:span 2}
.tl-item{display:flex;gap:8px;padding:4px 0;border-bottom:1px solid #1a1f40;font-size:.76em}
</style></head>
<body>
<div class="hdr">
  <div><h1>🤖 Parental Dashboard — Pepper Clinical v3.2</h1>
...
<div class="grid">
  <div class="card">
    ...
    <div style="display:flex;gap:14px;flex-wrap:wrap">
      <div style="text-align:center"><div class="num p" id="sc">0</div><div style="color:#6b7280;font-size:.73em">Score</div></div>
      <div style="text-align:center"><div class="num g" id="ms">0</div><div style="color:#6b7280;font-size:.73em">Mastered</div></div>
      <div style="text-align:center"><div class="num y" id="tk">0</div><div style="color:#6b7280;font-size:.73em">Tokens</div></div>
      <div style="text-align:center"><div class="num r" id="wr">0</div><div style="color:#6b7280;font-size:.73em">Wrong Now</div></div>
    </div>
  </div>
  <div class="card">
    <h3>Emotion</h3>
    <div style="font-size:2.6em;text-align:center" id="ee">😐</div>
    <div style="text-align:center;color:#a78bfa;font-size:1em;margin:4px 0" id="el">neutral</div>
    <div id="eb"></div>
  </div>
  <div class="card">
    <h3>Attention & Body</h3>
    <div style="display:flex;gap:12px;align-items:center">
      <div class="ring" id="ar" style="border-color:#34d399">70%</div>
      <div>
        <div style="color:#6b7280;font-size:.76em">Face</div>
        <div style="font-size:1.2em" id="fd">-</div>
        <div style="color:#6b7280;font-size:.76em;margin-top:5px">Fingers</div>
        <div style="font-size:1.2em;color:#fbbf24" id="fc">-</div>
        <div style="color:#6b7280;font-size:.76em;margin-top:5px">Motion</div>
        <div style="font-size:1.2em;color:#60a5fa" id="bm">-</div>
      </div>
    </div>
  </div>
  <div class="card">
    <h3>Current Task</h3>
    <div style="color:#a78bfa;font-size:.95em;margin-bottom:6px" id="tn">-</div>
    <div style="color:#6b7280;font-size:.76em">Domain</div>
    <div style="color:#60a5fa;font-size:.9em" id="td">-</div>
    <div style="color:#6b7280;font-size:.76em;margin-top:6px">Protocol</div>
    <div style="color:#34d399;font-size:.9em" id="tp">-</div>
    <div style="color:#6b7280;font-size:.76em;margin-top:6px">Consecutive</div>
    <div style="font-size:1.2em;color:#fbbf24" id="co">0/3</div>
  </div>
  <div class="card span2">
    <h3>Summary</h3>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
      <div><div style="color:#6b7280;font-size:.73em">Success</div><div class="num g" id="ok" style="font-size:1.6em">0</div></div>
      <div><div style="color:#6b7280;font-size:.73em">Fail</div><div class="num r" id="fl" style="font-size:1.6em">0</div></div>
      <div><div style="color:#6b7280;font-size:.73em">Auto</div><div class="num" id="au" style="font-size:1.6em;color:#fb923c">0</div></div>
      <div><div style="color:#6b7280;font-size:.73em">Streak</div><div class="num y" id="st" style="font-size:1.6em">0</div></div>
    </div>
  </div>
  <div class="card span2">
  ...
  <div class="card span2">
    ...
    <div id="ch" style="max-height:160px;overflow-y:auto"></div>
  </div>
</div>
<script>
...
@dash_app.route("/")
def _dash_root(): return render_template_string(DASH_HTML)
...
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',sans-serif;background:#050810;color:#e0e6ff;min-height:100vh}
.hdr{background:linear-gradient(135deg,#1a0a3d,#0a0f28);padding:14px 22px;border-bottom:2px solid #4f46e5}
.hdr h1{font-size:1.35em;color:#a78bfa}
.hdr p{color:#60a5fa;font-size:.80em;margin-top:3px}
.fbar{padding:10px 20px;display:flex;gap:7px;flex-wrap:wrap;background:#080a1a;border-bottom:1px solid #1a1f40}
.fb{padding:5px 13px;border-radius:16px;border:2px solid #1a1f40;background:#0c0f1e;
  color:#9ca3af;cursor:pointer;font-size:.78em;transition:all .2s}
.fb:hover,.fb.on{background:#4f46e5;color:white;border-color:#4f46e5}
.srch{padding:6px 13px;border-radius:16px;border:2px solid #1a1f40;background:#0c0f1e;
  color:#e0e6ff;font-size:.78em;width:180px}
.srch:focus{outline:none;border-color:#4f46e5}
.gg{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:10px;padding:14px}
.gc{background:#0c0f1e;border:1px solid #1a1f40;border-radius:13px;padding:14px;
  cursor:pointer;transition:all .2s;position:relative}
.gc:hover{border-color:#4f46e5;transform:translateY(-2px)}
.ge{font-size:2em;text-align:center;margin-bottom:5px}
.gn{font-size:.87em;font-weight:bold;color:#a78bfa;margin-bottom:2px}
.gd{font-size:.72em;color:#6b7280;margin-bottom:6px}
.gm{display:flex;gap:4px;flex-wrap:wrap}
.tag{padding:2px 6px;border-radius:6px;font-size:.64em;font-weight:bold}
...
.pb{position:absolute;top:9px;right:9px;background:#4f46e5;color:white;border:none;
  border-radius:6px;padding:3px 9px;font-size:.68em;font-weight:bold;cursor:pointer;
  opacity:0;transition:opacity .2s}
.gc:hover .pb{opacity:1}.pb:hover{background:#6366f1}
.sbar{position:fixed;bottom:0;left:0;right:0;background:#060918;
  border-top:1px solid #1a1f40;padding:8px 16px;display:flex;gap:16px;font-size:.78em}
...
.mbox{background:#0c0f1e;border:2px solid #4f46e5;border-radius:16px;
  padding:24px;max-width:440px;width:90%;text-align:center}
.mbox h2{color:#a78bfa;margin-bottom:8px}
.mbox p{color:#9ca3af;font-size:.86em;margin-bottom:14px}
.mac{display:flex;gap:8px;justify-content:center}
.bp{background:#4f46e5;color:white;border:none;border-radius:9px;
  padding:9px 20px;font-size:.86em;font-weight:bold;cursor:pointer}
.bc{background:#1a1f40;color:#9ca3af;border:none;border-radius:9px;
  padding:9px 20px;font-size:.86em;cursor:pointer}
</style></head>
<body>
<div class="hdr">
...
<div class="modal" id="mod">
  <div class="mbox">
    ...
    <div style="display:flex;gap:5px;justify-content:center;margin-bottom:12px" id="mt"></div>
    <div class="mac">
...
<script>
const GAMES=[
  {n:"Clap Hands",e:"👏",dom:"Motor",pr:"ABA",d:"Clap hands — motion detection"},
  {n:"Wave Hello",e:"👋",dom:"Motor",pr:"ESDM",d:"Wave to Pepper"},
  {n:"Raise Hand",e:"✋",dom:"Motor",pr:"DTT",d:"Raise hand high"},
  {n:"Touch Nose",e:"👆",dom:"Motor",pr:"TEACCH",d:"Touch nose with finger"},
  {n:"Arms Wide",e:"🤸",dom:"Motor",pr:"ESDM",d:"Arms wide like a bird"},
  {n:"Hands Up",e:"🙌",dom:"Motor",pr:"ABA",d:"Both hands up high"},
  {n:"Jump Up",e:"⬆️",dom:"Motor",pr:"ESDM",d:"Jump and bounce"},
  {n:"Stomp Feet",e:"🦶",dom:"Motor",pr:"DTT",d:"Stomp feet on floor"},
  {n:"Spin Around",e:"🌀",dom:"Motor",pr:"TEACCH",d:"Spin in circle"},
  {n:"March",e:"🪖",dom:"Motor",pr:"ESDM",d:"March left right"},
  {n:"Find Red",e:"🔴",dom:"Cognitive",pr:"ABA",d:"Touch the red color"},
  {n:"Find Blue",e:"🔵",dom:"Cognitive",pr:"DTT",d:"Touch the blue color"},
  ...
  {n:"Find Dog",e:"🐶",dom:"Cognitive",pr:"ABA",d:"Identify the dog"},
  {n:"Find Cat",e:"🐱",dom:"Cognitive",pr:"DTT",d:"Identify the cat"},
  {n:"Find Lion",e:"🦁",dom:"Cognitive",pr:"ESDM",d:"Find the lion"},
  {n:"Find Elephant",e:"🐘",dom:"Cognitive",pr:"TEACCH",d:"Find elephant"},
  {n:"Find Apple",e:"🍎",dom:"Cognitive",pr:"ABA",d:"Touch the apple"},
  {n:"Find Banana",e:"🍌",dom:"Cognitive",pr:"DTT",d:"Find banana"},
  {n:"Find Circle",e:"⭕",dom:"Cognitive",pr:"TEACCH",d:"Touch the circle"},
  {n:"Find Star",e:"⭐",dom:"Cognitive",pr:"ESDM",d:"Find the star"},
  {n:"Find Heart",e:"❤️",dom:"Cognitive",pr:"ABA",d:"Find the heart"},
  {n:"Happy Face",e:"😊",dom:"Social",pr:"ESDM",d:"Find the happy face"},
  {n:"Sad Face",e:"😢",dom:"Social",pr:"ESDM",d:"Find the sad face"},
  {n:"Angry Face",e:"😠",dom:"Social",pr:"ESDM",d:"Find angry face"},
  {n:"Surprised",e:"😲",dom:"Social",pr:"ESDM",d:"Find surprised face"},
  ...
  {n:"Count 10",e:"🔟",dom:"Math",pr:"DTT",d:"Show all 10 fingers"},
  {n:"Say Apple",e:"🗣️",dom:"Verbal",pr:"ABA",d:"Say the word APPLE"},
...
let CF='all',CG=null;
function filt(f,btn){CF=f;document.querySelectorAll('.fb').forEach(b=>b.classList.remove('on'));btn.classList.add('on');render();}
function render(){
...
function openM(g){CG=g;document.getElementById('me').textContent=g.e;document.getElementById('mn').textContent=g.n;document.getElementById('md').textContent=g.d;document.getElementById('mt').innerHTML=`<span class="tag ${TC[g.dom]||'ta'}">${g.dom}</span><span class="tag ${TC[g.pr]||'ta'}">${g.pr}</span>`;document.getElementById('mod').classList.add('open');}
function closeM(){document.getElementById('mod').classList.remove('open');CG=null;}
async function play(){if(!CG)return;try{await fetch('http://localhost:5001/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cmd:'next'})});closeM();alert('"'+CG.n+'" sent to Pepper!');}catch(e){alert('Pepper not running.');}}
async function loadSt(){try{const r=await fetch('http://localhost:5001/status').then(r=>r.json());document.getElementById('sc').textContent=r.child;document.getElementById('ss').textContent=r.score;document.getElementById('se').textContent=r.emotion;document.getElementById('sl').textContent=r.listening?'Yes':'No';}catch(e){}}
...
@games_app.route("/")
def _games_root(): return render_template_string(GAMES_HTML)
...
class Ctrl:
    ...
    def _say(self, text, wait=True):
        ...
        TTS.say(text)          # ← always uses TTS queue, never blocks Qt
        if wait:
            # Wait for speaking to finish, max 8s
            t0=time.time()
            while ST["is_speaking"] and time.time()-t0 < 8:
                time.sleep(0.1)
            time.sleep(0.2)
    ...
    def run(self):
        ...
        self._say(
            ...
            f"If you answer wrong {MAX_WRONG} times I will give you a new task. "
        ...
        while self.running:
            ...
            if task["domain"]=="Motor":
                ...
                threading.Thread(target=lambda:(
                    time.sleep(1.5), ST.update({"gaze":"child"})),
                    daemon=True).start()
            else:
            ...
            if timeout:
                ...
                if wc>=MAX_WRONG:
                    ...
                    TTS.say(emp("wrong2"))
                    ST["task_index"]=(ST["task_index"]+1)%len(POOL)
                    LOG(f"❌×{MAX_WRONG} → next task","warn")
                else:
                    ST["tasks_fail"]+=1
                    self._on_fail(task)
    ...
    def _run(self, task):
        """Returns (ok, timed_out, wrong_count)"""
    ...
    def _check(self, task):
        ...
        if mc in ["clap","wave","raise_hand","touch_nose",
            ...
            # ONLY check if heard_flag is set — prevents noise false fails
            if ST.get("heard_flag"):
                txt=ST.get("last_speech","")
                ST["heard_flag"]=False
                # Drain queue to clear any extra items
                while not ST["speech_q"].empty():
                    try:
                        qtxt=ST["speech_q"].get_nowait()
                        if kw in qtxt:
                            LOG(f"✅ Keyword '{kw}' from queue","success")
                            return "success"
                    except: break
                if kw in txt:
                    LOG(f"✅ Keyword '{kw}' heard","success")
                    return "success"
                # Only count as fail if they said something real (>2 chars)
                if len(txt) > 2:
                    LOG(f"❌ Said '{txt}' not '{kw}'","warn")
                    return "fail"
    ...
    def _on_ok(self, task):
        ...
        if ST["consecutive"]>=MASTERY_N:
            ...
            TTS.say(f"{CHILD_NAME} earned 3 stars! MASTERED! [LEVEL_UP] "
                    f"Now: {ntask.get('name','')}! [WAVE]")
    ...
    def _cmd(self):
        ...
        if cmd=="next":
            ...
            print(f"Mastered:{ST['tasks_mastered']} | "
                  f"OK:{ST['tasks_ok']} Fail:{ST['tasks_fail']} Auto:{ST.get('tasks_auto',0)}")
            print(f"CSV: {CSV_FILE}")
...
def main():
    print(f"""
╔══════════════════════════════════════════════════════════════════╗
║  PEPPER CLINICAL INFINITY v3.2 — {CHILD_NAME:<26}║
╠══════════════════════════════════════════════════════════════════╣
║  ✅ TTS: dedicated thread (no Qt conflict)                      ║
║  ✅ Emotion: 300x300, MediaPipe face mesh                       ║
║  ✅ Live: independent frame buffer (no freeze)                  ║
║  ✅ Speech: keyword-only fails (no false fails)                 ║
║  ✅ Tasks: {len(POOL)} endless ESDM/ABA/TEACCH/DTT                     ║
╠══════════════════════════════════════════════════════════════════╣
...
""")
    ...
    # Qt app (must be on main thread)
    qt=QApplication(sys.argv)
    qt.setApplicationName("Pepper Clinical v3.2")
    qt.setStyle("Fusion")
    pal=QPalette()
    ...
    # Camera capture (one thread owns cap)
    cap = CaptureThread(); cap.start()
    time.sleep(0.5)

    # MediaPipe analysis (reads from capture)
    mp_thread = MPThread(cap); mp_thread.start()

    # Tablet (Qt main thread)
    tablet = Tablet()
    tablet.set_capture(cap)
    tablet.show(); tablet.move(20,600)

    # Speech (always-on, own thread)
    speech = Speech(); speech.start()
    tablet.set_speech(speech)

    # Emotion window 300×300 (own thread)
...
✅ ALL SYSTEMS ACTIVE
  ...
  W1: PyBullet sim
  W2: Emotion — 300×300 fixed (MediaPipe)
  W3: Live Session — no freeze
  W4: Tablet 1280×810

  TTS: dedicated thread — Pepper can now talk! ✅
  Speech: always-on — word tasks always heard ✅
  ...
  Logic:  {MAX_WRONG} wrong = auto-advance + CSV log

Commands:
  n/next  skip   r/report  report   q/exit  quit
{'═'*64}
""")
