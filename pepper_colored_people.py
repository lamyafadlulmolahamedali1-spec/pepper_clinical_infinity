#!/usr/bin/env python3
"""
PEPPER WITH COLORED PEOPLE - غرفة مع مجسمات ملونة تمشي
"""
...
class ColoredPerson:
    ...
    def create_person(self):
        ...
        head_body = p.createMultiBody(baseMass=0.5, baseVisualShapeIndex=head, basePosition=[self.x, self.y, 0.9])
        self.body_parts.append(head_body)
        
        # الجسم (مستطيل)
        body = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.18, 0.12, 0.35], rgbaColor=self.color)
        body_body = p.createMultiBody(baseMass=0.5, baseVisualShapeIndex=body, basePosition=[self.x, self.y, 0.55])
        self.body_parts.append(body_body)
        
        # الذراع الأيمن
        right_arm = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.07, 0.07, 0.3], rgbaColor=self.color)
        right_arm_body = p.createMultiBody(baseMass=0.3, baseVisualShapeIndex=right_arm, basePosition=[self.x + 0.22, self.y, 0.7])
        ...
        left_arm_body = p.createMultiBody(baseMass=0.3, baseVisualShapeIndex=left_arm, basePosition=[self.x - 0.22, self.y, 0.7])
        ...
        right_leg_body = p.createMultiBody(baseMass=0.3, baseVisualShapeIndex=right_leg, basePosition=[self.x + 0.1, self.y, 0.25])
        ...
        left_leg_body = p.createMultiBody(baseMass=0.3, baseVisualShapeIndex=left_leg, basePosition=[self.x - 0.1, self.y, 0.25])
        self.body_parts.append(left_leg_body)
        
        return self.body_parts
    
    def walk(self, step=0.03):
        """تحريك المجسم"""
        self.x += step
        for part in self.body_parts:
            pos, ori = p.getBasePositionAndOrientation(part)
            p.resetBasePositionAndOrientation(part, [pos[0] + step, pos[1], pos[2]], ori)
    
    def set_position(self, x, y):
        self.x = x
        self.y = y
        for part in self.body_parts:
            p.resetBasePositionAndOrientation(part, [x, y, part_pos[2]], [0,0,0,1])
...
def create_full_room():
    ...
    # ===== طاولة في المنتصف =====
    table_top = p.createVisualShape(p.GEOM_BOX, halfExtents=[1.2, 0.9, 0.1], rgbaColor=[0.5, 0.3, 0.1, 1])
    p.createMultiBody(baseMass=0, baseVisualShapeIndex=table_top, basePosition=[0, 0, 0.55])
    
    # أرجل الطاولة
    for leg_x, leg_y in [(-0.9, -0.6), (-0.9, 0.6), (0.9, -0.6), (0.9, 0.6)]:
    ...
    # ===== كراسي =====
    chair_positions = [(-1.8, -1.5), (1.8, -1.5), (-1.8, 1.5), (1.8, 1.5)]
    ...
    print("✅ غرفة كاملة: طاولة وسط، كراسي، دولاب، مراية، بالونات!")
    return balloons
...
def move_colored_people(people):
    """تحريك المجسمات الملونة يمين ويسار"""
    directions = [0.02, -0.02, 0.015, -0.015]
    idx = 0
    while True:
        for i, person in enumerate(people):
            step = directions[(idx + i) % len(directions)]
            person.walk(step)
            # لو وصل للحدود، يغير الاتجاه
            if abs(person.x) > 3.5:
                person.walk(-step * 2)
        idx += 1
        time.sleep(0.05)
    ...
    while True:
        try:
            ...
            if cmd == 'q':
                ...
                speak("I see a table in the center, chairs, a cabinet, a mirror, colorful balloons, and colorful people walking!")
            else:
...
# ========== الرئيسي ==========
print("="*60)
print("🤖 PEPPER WITH COLORED PEOPLE - Starting...")
print("="*60)
...
# إعداد الكاميرا
p.resetDebugVisualizerCamera(cameraDistance=6.5, cameraYaw=50, cameraPitch=-30, cameraTargetPosition=[0, 0, 0.8])

# تشغيل qiBullet
sim_manager = SimulationManager()
client_id = sim_manager.launchSimulation(gui=False)

# وضع Pepper في مكان ثاني (ليس في منتصف الغرفة)
pepper = sim_manager.spawnPepper(client_id, translation=[-2.5, -2, 0])  # Pepper في الركن

# تهيئة الحركات
gestures = PepperGestures(pepper)

# إنشاء مجسمات ملونة (زي رسومات الأطفال)
colors_list = [
    [1, 0.2, 0.2, 1],  # أحمر
    [0.2, 1, 0.2, 1],  # أخضر
    [0.2, 0.2, 1, 1],  # أزرق
...
colored_people = []
names = ["Ahmed", "Sara", "Laila", "Omar", "Nour", "Khalid"]

for i in range(6):
    x = random.uniform(-2.5, 2.5)
    y = random.uniform(-2.5, 2.5)
    person = ColoredPerson(x, y, colors_list[i % len(colors_list)], names[i])
    colored_people.append(person)
    print(f"✅ {names[i]} (مجسم ملون) تم إنشاؤه!")
...
print("\n✅ Pepper في الركن (x=-2.5, y=-2)")
print("✅ الطاولة في المنتصف (x=0, y=0)")
print("✅ 6 مجسمات ملونة تمشي في الغرفة!")
speak("Hello! I am Pepper! I am in the corner! Look at the colorful people walking around!")
