...
"""
╔══════════════════════════════════════════════════════════════════╗
║  PEPPER CLINICAL INFINITY V2 — FINAL COMPLETE SYSTEM           ║
║  Commander: Lamya | Omdurman Islamic University                 ║
╠══════════════════════════════════════════════════════════════════╣
║  FIXES:                                                          ║
║  ✅ Emotion detection FIXED (MediaPipe FaceMesh geometry)      ║
║  ✅ Emotion window 300×300                                      ║
║  ✅ Pepper avatar in Live Session (animated, lip-sync)         ║
║  ✅ Full-body + 10-finger detection (high accuracy)            ║
║  ✅ 2 wrong answers → report → next task (DTT protocol)       ║
║  ✅ Endless exercises (ABA/DTT/TEACCH/ESDM)                    ║
║  ✅ Mic FIXED — kids can speak in verbal tasks                 ║
║  ✅ Text chat in Live Session (Q&A + YouTube open)            ║
║  ✅ Parent dashboard 5007/5001 + Games 5009                    ║
...
from PyQt6.QtWidgets import (
    ...
    QLabel, QPushButton, QFrame, QGridLayout, QLineEdit, QScrollArea,
...
MIC_ENERGY=150    # High sensitivity for kids
MASTERY_N=3       # Consecutive successes
MAX_FAILS=2       # Wrong answers before skip + report
...
# ── Multi-patient ──────────────────────────────────────────────
print("\n"+"═"*60)
print("  PEPPER CLINICAL INFINITY V2 — Commander: Lamya")
print("═"*60)
...
with open(CSV_FILE,'w',newline='') as f:
    csv.writer(f).writerow(["Time","Child","Task","Domain","Level","Result",
...
def log_csv(task_id,domain,level,result,consec,fails,score,emotion,att):
    with open(CSV_FILE,'a',newline='') as f:
        csv.writer(f).writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            "SUCCESS" if result else "FAIL",consec,fails,score,emotion,att])

# ═══════════════════════════════════════════════════════════════
# TASK GENERATOR — Endless ABA/DTT/TEACCH/ESDM
# ═══════════════════════════════════════════════════════════════
def _fig(mov,size=260):
    ...
    if mov=="clap":
        ...
        nx,ny=cx,cy-56; dr.ellipse([nx-7,ny-7,nx+7,ny+7],fill=hl)
...
FRUITS=[{"id":"apple","emoji":"🍎","label":"Apple"},{"id":"banana","emoji":"🍌","label":"Banana"},
        ...
        {"id":"strawberry","emoji":"🍓","label":"Strawberry"},{"id":"watermelon","emoji":"🍉","label":"Watermelon"},
        {"id":"mango","emoji":"🥭","label":"Mango"},{"id":"pineapple","emoji":"🍍","label":"Pineapple"}]
...
MOTORS=[
    {"id":"clap","name":"👏 Clap","verify":"clap","prompts":["Clap!","Hands together!","*clap clap*"],
     "instruction":"Look at the screen and CLAP your hands!","waiting":"I am waiting — clap! 👏",
     "success":"AMAZING! You clapped correctly! ✅","fail":"Not quite! Clap your hands together!"},
    {"id":"wave","name":"👋 Wave","verify":"wave","prompts":["Wave!","Side to side!","Hello!"],
     "instruction":"Look at the screen and WAVE hello!","waiting":"Wave your hand! 👋",
     "success":"WONDERFUL! You waved! ✅","fail":"Wave your hand side to side!"},
    {"id":"raise_hand","name":"✋ Raise Hand","verify":"raise_hand","prompts":["Hand up!","Higher!","Reach up!"],
     "instruction":"Look at the screen and RAISE your hand HIGH!","waiting":"Raise your hand! ✋",
     "success":"PERFECT! Hand up high! ✅","fail":"Lift your arm above your head!"},
    {"id":"touch_nose","name":"👆 Touch Nose","verify":"touch_nose","prompts":["Touch nose!","Finger here!","Nose!"],
     "instruction":"Look at the screen and TOUCH your NOSE!","waiting":"Touch your nose! 👆",
     "success":"BRILLIANT! You touched your nose! ✅","fail":"Point your finger to your nose!"},
    {"id":"arms_out","name":"🤸 Arms Wide","verify":"arms_out","prompts":["Arms out!","Like airplane!","Wide!"],
     "instruction":"Stretch BOTH arms out WIDE like an airplane!","waiting":"Spread your arms! 🤸",
     "success":"AMAZING! Like an airplane! ✅","fail":"Spread both arms wide to the sides!"},
    {"id":"hands_up","name":"🙌 Hands Up","verify":"hands_up","prompts":["Both hands!","Up up!","Two hands!"],
     "instruction":"Put BOTH hands UP HIGH!","waiting":"Both hands up! 🙌",
     "success":"SUPERSTAR! Both hands up! ✅","fail":"Lift BOTH arms above your head!"},
    {"id":"point","name":"👉 Point","verify":"point","prompts":["Point!","Finger out!","Straight out!"],
     "instruction":"POINT your finger forward!","waiting":"Point your finger! 👉",
     "success":"GREAT pointing! ✅","fail":"Stretch your index finger forward!"},
]
WORDS=["apple","ball","cat","dog","elephant","fish","good","happy","jump",
       ...
       "four","five","six","seven","eight","nine","ten","mama","papa","baby",
       "book","cup","door","ear","eye","foot","hand","head","leg","nose","arm"]
...
def gen_tasks(n=3000):
    ...
    for _ in range(int(n*0.12)):
        ...
        pool.append({"id":f"color_{tgt['id']}","domain":"Cognitive","level":2,
            ...
            "waiting":f"Find and click {tgt['label']}!",
            "success":f"CORRECT! That is {tgt['label']}! ✅","fail":f"Not that one! Find {tgt['label']}!",
            "tablet_mode":"color_grid","options":opts,"correct":cor,"tokens":3,"joy":"celebrate",
    ...
    for _ in range(int(n*0.12)):
        ...
        pool.append({"id":f"animal_{tgt['id']}","domain":"Cognitive","level":3,
            ...
            "waiting":f"Find the {tgt['label']}!",
            "success":f"YES! The {tgt['label']}! ✅","fail":f"Not that one! Find {tgt['label']}!",
            "tablet_mode":"object_grid","options":opts,"correct":cor,"tokens":4,"joy":"dance",
            "prompts":[f"Find the {tgt['label']}!","Look again!","Which one is the {tgt['label']}?"]})
    # Fruits (11%)
    for _ in range(int(n*0.11)):
        ...
        pool.append({"id":f"fruit_{tgt['id']}","domain":"Cognitive","level":4,
            "name":f"🍎 {tgt['label']}","instruction":f"Click the {tgt['label']}!",
            "waiting":f"Find the {tgt['label']}!",
            "success":f"DELICIOUS {tgt['label']}! ✅","fail":f"Look again! Find the {tgt['label']}!",
            "tablet_mode":"object_grid","options":opts,"correct":cor,"tokens":4,"joy":"celebrate",
            "prompts":[f"Find {tgt['label']}!","Look carefully!","You can do it!"]})
    # Shapes (8%)
    for _ in range(int(n*0.08)):
        ...
        pool.append({"id":f"shape_{tgt['id']}","domain":"Cognitive","level":5,
            "name":f"🔷 {tgt['label']}","instruction":f"Click the {tgt['label']}!",
            "waiting":f"Find the {tgt['label']}!",
            "success":f"CORRECT SHAPE! {tgt['label']}! ✅","fail":f"Find the {tgt['label']}!",
            "tablet_mode":"object_grid","options":opts,"correct":cor,"tokens":4,"joy":"dance",
            "prompts":[f"Find the {tgt['label']}!","Look carefully!","Which shape?"]})
    # Math (10%)
    for _ in range(int(n*0.10)):
        ...
        pool.append({"id":f"count_{num}","domain":"Math","level":5,
            ...
            "waiting":f"Hold up {num} finger{'s' if num>1 else ''}! 🖐️",
            "success":f"YES! {num} finger{'s' if num>1 else ''}! Amazing! ✅",
            "fail":f"Try again! I need to see {num} fingers!",
            "tablet_mode":"number_display","target_number":num,
            "verify":"finger_count","tokens":5,"joy":"celebrate",
            "prompts":[f"Show {num} fingers!","Count with me!",f"{num}!"]})
    # Verbal (17%)
    for _ in range(int(n*0.17)):
        ...
        pool.append({"id":f"say_{word}","domain":"Verbal","level":6,
            ...
            "waiting":f"Say {word}! 🎤 Tap the mic and say it!",
            "success":f"I HEARD {word.upper()}! Perfect! ✅",
            "fail":f"Try again! Tap mic and say: {word}!",
            "tablet_mode":"word_display","word_emoji":"🗣️","word_text":word.upper(),
            "verify":"speech_keyword","keyword":word,"tokens":4,"joy":"dance",
            "prompts":[f"Say {word}!",f"Tap mic → say {word}!",f"{word[0]}...{word}!"]})
...
ST={
    ...
    "tablet_click_result":None,"tablet_correct":-1,"tablet_instruction":"Welcome!",
    # Vision
    "emotion":"neutral","emotion_conf":0.0,"face_detected":False,
    ...
    "blink_timer":0.0,"head_tilt_val":0.0,
    # Progress
    ...
    "att_history":[],"score_history":[],"emo_history":[],
...
class TabletBridge(QObject):
    ...
    sig_celebration  = pyqtSignal()
    sig_rec_start    = pyqtSignal()
    sig_rec_stop     = pyqtSignal(str)
    sig_chat_msg     = pyqtSignal(str,str) # role, text
...
class CameraThread(QThread):
    ...
    def _emotion_from_mesh(self,lm,w,h):
        """
        FIXED EMOTION DETECTION — uses actual facial geometry from FaceMesh.
        ...
        try:
            ...
            ear_avg=(ear(r_eye)+ear(l_eye))/2.0
            ...
            l_brow=lm[107]; l_eye_top=lm[159]
            r_brow=lm[336]; r_eye_top=lm[386]
            brow_raise=((l_brow.y-l_eye_top.y)+(r_brow.y-r_eye_top.y))/2.0
            # Negative brow_raise = brows raised (surprise)
            # Positive = frown (angry/sad)

            # Cheek raise (smile indicator)
            l_cheek=lm[116]; r_cheek=lm[345]
            nose_tip=lm[4]
            cheek_height=((l_cheek.y+r_cheek.y)/2.0-nose_tip.y)*h

            # Decision tree
            if ear_avg<0.17:
                return "surprised", 0.85
            elif mar>0.45 and cheek_height<-5:
                return "happy", 0.88
            elif mar>0.30:
                return "joyful", 0.75
            elif brow_raise>0.02 and mar<0.20:
                return "angry", 0.72
            elif brow_raise>0.01 and mar<0.25:
                return "sad", 0.70
            elif ear_avg<0.22 and mar<0.18:
                return "fear", 0.65
            elif mar>0.25 and ear_avg>0.25:
                return "happy", 0.70
            else:
                return "neutral", 0.80
        except:
    ...
    def _validate_motor(self):
        ...
        if va=="clap": ok=ST["clapping"]
            ...
            nx=fm.get("nose_x",-1); fw=fm.get("frame_w",640)
            ny=fm.get("nose_y",-1); fh=fm.get("frame_h",480)
            if nx>0 and pl:
            ...
            lsa=pl.get("l_shoulder_angle",0); rsa=pl.get("r_shoulder_angle",0)
            ok=(lsa>65 and rsa>65)
        elif va=="hands_up":
            ok=ST.get("hands_up",False)
        elif va=="point":
            if pl:
                rix=pl.get("r_index_x",0); rwy=pl.get("r_wrist_y",1)
                ok=(rix>0.58 and abs(pl.get("r_index_y",0)-rwy)<0.12)
    ...
    def _sim_frame(self):
        ...
        cv2.putText(f,"NO CAMERA — SIMULATION",
            (160,240),cv2.FONT_HERSHEY_SIMPLEX,0.7,(100,150,255),2)
        return f
    ...
    def run(self):
        ...
        while self.running:
                ...
                try:
                    ...
                    if res.pose_landmarks:
                        ...
                        pl={
                            ...
                            "l_knee_y":lm[PL.LEFT_KNEE].y,"r_knee_y":lm[PL.RIGHT_KNEE].y,
            ...
            # ── FaceMesh — EMOTION DETECTION FIXED ──────────
            if self._face:
                try:
                    rgb=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
            ...
            # Update emotion history periodically
            if int(time.time())%3==0:
                ST["att_history"].append(ST["attention"])
                ST["emo_history"].append(ST["emotion"])
                if len(ST["att_history"])>60: ST["att_history"]=ST["att_history"][-60:]
                if len(ST["emo_history"])>60: ST["emo_history"]=ST["emo_history"][-60:]
...
# ═══════════════════════════════════════════════════════════════
# EMOTION WINDOW (OpenCV) — 300×300 FIXED
# ═══════════════════════════════════════════════════════════════
class EmotionWindow:
    ...
    def _run(self):
        ...
        while self.running:
            ...
            em=ST["emotion"]; conf=ST.get("emotion_conf",0.0)
            ...
            # Big emotion circle
            cv2.circle(f,(150,100),55,col,-1)
            cv2.circle(f,(150,100),55,(255,255,255),2)
            em_icons={"happy":"😊","joyful":"😄","sad":"😢","angry":"😠",
                      "fear":"😨","surprised":"😲","neutral":"😐"}
            cv2.putText(f,em.upper(),
                (150-len(em)*7,105),cv2.FONT_HERSHEY_SIMPLEX,0.65,
                (255,255,255),2)
            # Confidence bar
            bw=int(conf*296)
            cv2.rectangle(f,(2,162),(298,178),(25,28,50),-1)
            cv2.rectangle(f,(2,162),(2+bw,178),col,-1)
            cv2.putText(f,f"Conf: {conf:.0%}",
                (8,193),cv2.FONT_HERSHEY_SIMPLEX,0.42,(200,200,200),1)
            # Emotion bars — all emotions
            emos=list(self.ECOL.keys())
            emo_scores={}
            em2=ST["emotion"]
            for e_ in emos:
                emo_scores[e_]=0.05
            emo_scores[em2]=conf
            y0=205
            for e_,sc_ in emo_scores.items():
                bar=int(sc_*140); ec_=self.ECOL.get(e_,(150,150,150))
                cv2.rectangle(f,(2,y0),(145,y0+12),(22,25,45),-1)
                if bar>0: cv2.rectangle(f,(2,y0),(2+bar,y0+12),ec_,-1)
                cv2.putText(f,e_[:6],(150,y0+10),cv2.FONT_HERSHEY_SIMPLEX,0.32,(200,200,200),1)
                y0+=14
            # Face detected indicator
            fd_col=(0,255,0) if ST["face_detected"] else (0,0,200)
            cv2.circle(f,(285,20),10,fd_col,-1)
            cv2.putText(f,"Face" if ST["face_detected"] else "No Face",
                (220,18),cv2.FONT_HERSHEY_SIMPLEX,0.30,fd_col,1)
            # Finger count
            fc=ST["finger_count"]
            if fc>0:
                cv2.putText(f,f"Fingers:{fc}  {'|'*fc}",
                    (5,296),cv2.FONT_HERSHEY_SIMPLEX,0.38,(255,200,0),1)
...
# ═══════════════════════════════════════════════════════════════
# LIVE SESSION WINDOW (OpenCV) — Avatar + body tracking
# ═══════════════════════════════════════════════════════════════
class LiveWindow:
    ...
    def _draw_avatar(self,win,cx,cy,lip,em,fc,social_joy):
        """Animated Pepper avatar with lip-sync + micro-animations"""
        self._phase+=0.12 if ST["is_speaking"] else 0.03
        # Body
        cv2.ellipse(win,(cx,cy+80),(46,66),0,0,360,(80,100,200),-1)
        cv2.ellipse(win,(cx,cy+80),(46,66),0,0,360,(100,120,220),2)
        # Arms
        if social_joy:
            jt=time.time()
            la=int(28*math.sin(jt*4)); ra=int(28*math.sin(jt*4+1))
            cv2.ellipse(win,(cx-56+la,cy+26),(11,50),-60+la,0,360,(70,90,190),-1)
            cv2.ellipse(win,(cx+56+ra,cy+26),(11,50),60+ra,0,360,(70,90,190),-1)
        elif ST["is_speaking"]:
            la=int(20*math.sin(self._phase))
            ra=int(20*math.sin(self._phase+math.pi))
            cv2.ellipse(win,(cx-56+la,cy+62),(11,35),-28+la,0,360,(70,90,190),-1)
            cv2.ellipse(win,(cx+56+ra,cy+62),(11,35),28+ra,0,360,(70,90,190),-1)
        else:
            cv2.ellipse(win,(cx-56,cy+66),(11,30),-14,0,360,(70,90,190),-1)
            cv2.ellipse(win,(cx+56,cy+66),(11,30),14,0,360,(70,90,190),-1)
        # Head
        hbob=int(3*math.sin(self._phase*0.5))
        ht=ST.get("head_tilt_val",0.0)*8
        hy=cy-50+hbob; hx=cx+int(ht)
        cv2.circle(win,(hx,hy),50,(220,195,173),-1)
        cv2.circle(win,(hx,hy),50,(200,175,155),2)
        # Eyes
        blink=ST.get("blinking",False) or (int(self._phase*3)%44==0)
        ey=5 if not blink else 1
        ec=ST["eye_color"] if social_joy else (100,180,255)
        for ex_ in [hx-17,hx+17]:
            cv2.ellipse(win,(ex_,hy-8),(6,ey),0,0,360,ec,-1)
            if not blink:
                cv2.circle(win,(ex_+1,hy-8),2,(255,255,255),-1)
        # Nose twitch
        nt=ST.get("nose_twitch",0.0) if hasattr(ST,"nose_twitch") else 0
        cv2.circle(win,(hx,hy+7),4,(180,140,120),-1)
        # Mouth lip-sync
        mh=int(4+lip*16)
        if ST["is_speaking"]:
            cv2.ellipse(win,(hx,hy+20),(14,mh),0,0,180,(160,80,80),-1)
            cv2.ellipse(win,(hx,hy+20),(14,mh),0,0,180,(210,110,110),2)
        elif em in ["happy","joyful"]:
            cv2.ellipse(win,(hx,hy+18),(14,7),0,0,180,(150,80,80),-1)
        else:
            cv2.line(win,(hx-11,hy+19),(hx+11,hy+19),(150,80,80),2)
        # Ears
        for ex_ in [hx-48,hx+48]:
            cv2.circle(win,(ex_,hy-5),9,(210,185,163),-1)
            cv2.circle(win,(ex_,hy-5),5,(240,200,180),-1)
        # Status ring
        if social_joy:
            jt=time.time(); pr=80+int(14*abs(math.sin(jt*4)))
            jc=(int(128+127*math.sin(jt*3)),int(200+55*math.sin(jt*2)),255)
            cv2.circle(win,(cx,cy),pr,jc,3)
            cv2.putText(win,"SOCIAL JOY!",(cx-50,cy+182),
                cv2.FONT_HERSHEY_SIMPLEX,0.50,(255,220,0),2)
        elif ST["is_speaking"]:
            pr=86+int(5*math.sin(self._phase*5))
            cv2.circle(win,(cx,cy),pr,(0,160,255),2)
            cv2.putText(win,f"SPEAKING",(cx-38,cy+182),
                cv2.FONT_HERSHEY_SIMPLEX,0.44,(0,200,255),1)
        elif ST["recording"]:
            cv2.circle(win,(cx,cy),86,(0,0,255),2)
            cv2.putText(win,"RECORDING",(cx-44,cy+182),
                cv2.FONT_HERSHEY_SIMPLEX,0.46,(0,0,255),2)
        elif ST["listening"]:
            cv2.circle(win,(cx,cy),86,(0,220,100),2)
            cv2.putText(win,"LISTENING",(cx-44,cy+182),
                cv2.FONT_HERSHEY_SIMPLEX,0.46,(0,220,100),2)
        else:
            cv2.putText(win,"READY",(cx-25,cy+182),
                cv2.FONT_HERSHEY_SIMPLEX,0.44,(100,100,150),1)
        ...
        while self.running:
            ...
            if self.cam.cap and self.cam.cap.isOpened():
                ...
                if ret and frame is not None:
                    ...
                    cam_r=cv2.resize(frame,(430,340))
                    win[100:440,5:435]=cam_r
                    cv2.rectangle(win,(5,100),(435,440),(60,65,120),2)
            else:
                cv2.rectangle(win,(5,100),(435,440),(40,45,80),2)
                cv2.putText(win,"NO CAMERA",(150,270),cv2.FONT_HERSHEY_SIMPLEX,0.8,(100,150,255),2)
            ...
            # Avatar (right side)
            self._draw_avatar(win,740,190,lip,em,fc,sj)

            # Body tracking stats below camera
            suc=ST["consecutive"]
            cv2.putText(win,em.upper(),(14,460),cv2.FONT_HERSHEY_SIMPLEX,0.58,col,2)
            cv2.putText(win,f"{'★'*suc}{'☆'*(3-suc)} {suc}/3",
                (14,478),cv2.FONT_HERSHEY_SIMPLEX,0.44,(255,220,0),1)
            cv2.putText(win,f"Motion:{ST.get('body_motion',0):.1f}",
                (14,496),cv2.FONT_HERSHEY_SIMPLEX,0.38,(200,200,200),1)
            if fc>0:
                cv2.putText(win,f"Fingers:{fc} {'|'*min(fc,10)}",
                    (14,512),cv2.FONT_HERSHEY_SIMPLEX,0.42,(255,200,0),1)
            # Gesture badges
            ...
            for gi,(lbl,active,ac) in enumerate(acts):
                ...
                cv2.rectangle(win,(xp,528),(xp+104,550),bg,-1)
                cv2.rectangle(win,(xp,528),(xp+104,550),ac if active else (45,50,70),1)
                cv2.putText(win,lbl,(xp+5,544),cv2.FONT_HERSHEY_SIMPLEX,0.42,
                    ac if active else (55,60,70),1)

            # Recent chat (bottom strip)
            cy_=555
            for msg in ST["session_chat"][-3:]:
                isp=msg["role"]=="pepper"
                txt=msg["text"][:55]+("…" if len(msg["text"])>55 else "")
                cv2.rectangle(win,(5,cy_-14),(580,cy_+4),
                    (28,18,58) if isp else (8,28,14),-1)
                cv2.rectangle(win,(5,cy_-14),(580,cy_+4),
                    (90,70,190) if isp else (0,170,70),1)
                pfx="🤖 " if isp else f"👦 "
                cv2.putText(win,pfx+txt,(10,cy_),
            ...
            cv2.putText(win,f"LIVE SESSION — {CHILD_NAME} | Pepper Clinical Infinity V2",
            ...
            cv2.putText(win,
                ...
                f"Streak:{ST['streak']} | Att:{ST['attention']}%",
...
# ═══════════════════════════════════════════════════════════════
# TOUCH RECORDER — high-sensitivity, FIXED for verbal tasks
# ═══════════════════════════════════════════════════════════════
class TouchRecorder:
    def __init__(self):
    ...
    def stop_and_recognise(self)->str:
        ...
        if self.whisper:
            try:
                ...
                os.unlink(tp)
                if text: LOG(f"Whisper: {text}","info"); return text.lower()
            except Exception as e: LOG(f"Whisper err: {e}","warn")
        # Google fallback
        try:
            text=self.r.recognize_google(audio); LOG(f"Google: {text}","info")
            return text.lower()
        except sr.UnknownValueError: pass
        except Exception as e: LOG(f"SR: {e}","warn")
        return ""
    ...
    def _set_normal(self):
        ...
        if m=="color_grid":
            ...
            self.setStyleSheet(f"""QPushButton{{background:{d['color']};
                border-radius:74px;border:5px solid rgba(255,255,255,0.3);
                color:white;font-weight:bold;text-shadow:1px 1px 4px rgba(0,0,0,0.9);}}
...
# ═══════════════════════════════════════════════════════════════
# TABLET WINDOW (W4) — PyQt6 FIXED 1300×800
# ═══════════════════════════════════════════════════════════════
class TabletWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Pepper Clinical Infinity V2 — {CHILD_NAME}")
    ...
    def _setup_ui(self):
        ...
        # Camera
        self.cam_lbl=QLabel(); self.cam_lbl.setFixedSize(640,420)
        ...
        self.finger_lbl.setFont(QFont("Arial",11,QFont.Weight.Bold))
        self.finger_lbl.setStyleSheet("color:#fbbf24;padding:4px;background:#07090f;")
        self.finger_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ll.addWidget(self.finger_lbl)

        # ── TEXT CHAT AREA ─────────────────────────────────
        chat_hdr=QLabel("💬 Chat with Pepper — Ask anything or answer tasks!")
        chat_hdr.setFont(QFont("Arial",9,QFont.Weight.Bold))
        chat_hdr.setStyleSheet("color:#a78bfa;background:#0c0f1e;padding:4px;")
        ...
        self.chat_area.setStyleSheet("""QTextEdit{background:#07090f;color:#e0e6ff;
            border:1px solid #1a1f40;border-radius:0px;padding:4px;}""")
        self.chat_area.setFixedHeight(148)
        ll.addWidget(self.chat_area)

        chat_input_row=QWidget()
        cir=QHBoxLayout(chat_input_row); cir.setContentsMargins(4,2,4,2); cir.setSpacing(4)
        self.chat_input=QLineEdit()
        self.chat_input.setPlaceholderText("Type question or answer here... (Enter to send)")
        ...
        send_btn.setStyleSheet("QPushButton{background:#4f46e5;color:white;border-radius:8px;font-weight:bold;}")
        send_btn.clicked.connect(self._send_chat)
        cir.addWidget(send_btn)
        ll.addWidget(chat_input_row)
        ...
        # Header
        hdr=QFrame(); hdr.setFixedHeight(70)
        ...
        self.av_lbl=QLabel("🤖")
        self.av_lbl.setFont(QFont("Arial",24)); self.av_lbl.setStyleSheet("color:#a78bfa;")
        hl.addWidget(self.av_lbl)
        tw_=QWidget(); tl2=QVBoxLayout(tw_); tl2.setSpacing(1)
        self.title_lbl=QLabel("PEPPER CLINICAL INFINITY V2")
        ...
        self.stars_lbl.setFont(QFont("Arial",15)); self.stars_lbl.setStyleSheet("color:#4b5563;")
        ...
        self.content_fr.setMinimumHeight(260)
        ...
        self.mic_btn.setFixedHeight(56)
        self.mic_btn.setFont(QFont("Arial",13,QFont.Weight.Bold))
        self.mic_btn.setStyleSheet("""
        QPushButton{background:qlineargradient(x1:0,y1:0,x2:1,y2:0,
            stop:0 #dc2626,stop:1 #ef4444);
            color:white;border-radius:28px;border:3px solid #fca5a5;}
        ...
        # Rec status
        self.rec_status=QLabel("🎤 Tap mic button to speak — works for ALL verbal tasks!")
        ...
        sb.setStyleSheet("QFrame{background:#07090f;border-radius:8px;border:1px solid #1a1f40;}")
        ...
        self.lock_ov.setGeometry(0,0,636,260)
    ...
    def _connect_bridge(self):
        ...
        BRIDGE.sig_show_skip.connect(self._on_skip_overlay)
    ...
    def _on_new_task(self,data):
        ...
        self.fb_lbl.setText("Your turn! Click or perform the task!")
        self.fb_lbl.setStyleSheet("color:#60a5fa;")
    ...
    def _build_content(self,data):
        ...
        if mode=="motor_model":
            ...
            if fig and "base64," in fig:
                ...
                pix=QPixmap.fromImage(qi).scaled(200,200,
    ...
    def _on_skip_overlay(self,msg):
        """Show skip notification with mini report"""
    ...
    def _on_rec_stop(self,text):
        if text:
            ...
            ST["session_chat"].append({"role":"child","text":text,
                "time":datetime.now().strftime("%H:%M:%S")})
            if len(ST["session_chat"])>60: ST["session_chat"]=ST["session_chat"][-60:]
            self._add_chat_message("child",text)
            LOG(f"Heard: {text}","info")
        else:
            self.rec_status.setText("❌ Could not hear — try again! Speak closer to mic")
            self.rec_status.setStyleSheet("color:#f87171;padding:1px;")
        QTimer.singleShot(3500,lambda: self.rec_status.setText(
            "🎤 Tap mic button to speak — works for ALL verbal tasks!"))
    ...
    def _process_chat(self,text):
        """Handle text chat — YouTube, Q&A, task answers"""
        t=text.lower()
        # YouTube request
        yt_kw=["how to","show me","teach me","youtube","video","watch"]
        if any(kw in t for kw in yt_kw):
            query=text.replace("how to","").replace("show me","").replace("teach me","").strip()
            url=f"https://www.youtube.com/results?search_query={query.replace(' ','+')}+for+kids"
            import webbrowser; webbrowser.open(url)
            reply=f"Opening a YouTube video about '{query}' for you! 🎬"
            BRIDGE.sig_chat_msg.emit("pepper",reply)
            BRIDGE.sig_set_feedback.emit(f"📺 YouTube: {query}")
        ...
        tidx=min(ST["task_index"],len(TASK_POOL)-1)
        task=TASK_POOL[tidx]
        if task.get("verify")=="speech_keyword":
            kw=task.get("keyword","")
            if kw and kw in t:
                reply=f"PERFECT! I heard {kw}! ✅"
                BRIDGE.sig_chat_msg.emit("pepper",reply)
                return
        # General Q&A via Gemini or empathy
    ...
    def _quick_reply(self,text):
        t=text.lower()
        if any(w in t for w in ["hello","hi","hey"]): return f"Hello {CHILD_NAME}! 👋 I am so happy to talk with you!"
        if any(w in t for w in ["how are you","are you okay"]): return "I am great! Ready to learn with you! 😊"
        if any(w in t for w in ["good","great","yes","okay"]): return f"Wonderful {CHILD_NAME}! Let us continue! 🌟"
        if any(w in t for w in ["help","don't know","dont know","confused"]): return "No worries! I will show you again. Watch carefully! 🎯"
        if any(w in t for w in ["tired","stop","break"]): return "Okay! Short break time! Rest and come back! 💙"
        if any(w in t for w in ["good morning"]): return f"Good morning {CHILD_NAME}! Ready to learn amazing things today? ☀️"
        if any(w in t for w in ["good night"]): return f"Good night {CHILD_NAME}! You did amazing today! 🌙⭐"
        return f"That is interesting {CHILD_NAME}! Let us keep learning! 🎯"
    ...
    def _add_chat_message(self,role,text):
        ...
        if role=="child":
            ...
            color="#a78bfa"
            name="Pepper 🤖"
        self.chat_area.append(f'<span style="color:{color};font-weight:bold">{name}:</span> '
...
class PepperSim:
    def __init__(self):
        self.pepper=None; self.ok=False; self.rx=self.ry=0.0
    ...
    def launch(self):
        try:
            ...
            self.ok=True
            for fn in [self._arm_loop,self._walk_loop]:
        ...
        while True:
            try:
                ...
                if ST["is_speaking"]:
                    ...
                    self._sa("HeadPitch",-0.04-lip*0.13,.20)
                elif ST["recording"] or ST["listening"]:
                    self._sa("HeadYaw",0.25,.06); self._sa("HeadPitch",0.12,.06)
                elif ST.get("social_joy_active",False):
                    jt=time.time()
                    self._sa("LShoulderPitch",0.05+0.3*abs(math.sin(jt*4)),.25)
                    self._sa("RShoulderPitch",0.05+0.3*abs(math.sin(jt*4+math.pi)),.25)
                    self._sa("HeadPitch",-0.25+0.1*math.sin(jt*.5),.15)
                    ...
                    if ST.get("blinking",False): self._sa("HeadPitch",0.07,.12)
                    else: self._sa("HeadPitch",0.0,.03)
            except: pass
            time.sleep(.04)
    ...
    def _walk_loop(self):
        ...
        while True:
            ...
            try:
                if self.ok: self.pepper.setPosition([self.rx,self.ry,.8])
            except: pass
            time.sleep(.06)
    ...
    def show_text(self,text):
        ...
        try:
            ...
            self.p.addUserDebugText(text[:55],[pos[0],pos[1],pos[2]+1.35],
                [0,0,0],textSize=.80,lifeTime=5)
        except: pass

# ═══════════════════════════════════════════════════════════════
# FLASK SERVERS — Parent Dashboard (5007, 5001) + Games (5009)
# ═══════════════════════════════════════════════════════════════
...
.nav a{padding:6px 12px;border-radius:8px;text-decoration:none;
       font-size:.78em;font-weight:700;display:flex;align-items:center}
...
.n{font-size:1.6em;font-weight:700;color:#a78bfa}.l{font-size:.65em;color:#6b7280;margin-top:2px}
...
.emo-box{display:inline-block;padding:4px 12px;border-radius:12px;font-weight:700;font-size:.9em}
...
.bar-bg{background:#1f2937;border-radius:5px;height:8px;margin:3px 0}
.bar{height:8px;border-radius:5px;background:linear-gradient(90deg,#6366f1,#a78bfa)}
.log-box{max-height:200px;overflow-y:auto;scrollbar-width:thin}
...
.skill{text-align:center;background:#07090f;border-radius:8px;padding:8px}
.skill-val{font-size:1.2em;font-weight:700;color:#a78bfa}
...
.note{background:#0a1020;border-left:3px solid #a78bfa;padding:5px;
      margin:2px 0;font-size:.72em}
...
<div class="nav">
  ...
  <a href="http://{{local_ip}}:5007/" class="b4">📱 Android</a>
</div>
<div class="row">
  ...
  <div class="stat"><div class="n n-b">{{attention}}%</div><div class="l">🎯 Attention</div></div>
  <div class="stat"><div class="n n-r">{{skipped}}</div><div class="l">⏭️ Skipped</div></div>
</div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
  <div>
    <div class="card"><h2>😊 Current Emotion</h2>
      <div style="text-align:center;padding:8px">
        <div class="emo-box {{emotion}}">{{emotion.upper()}}</div>
        <div style="margin:6px 0"><div class="bar-bg"><div class="bar" style="width:{{attention}}%"></div></div>
          <span style="font-size:.65em;color:#a78bfa">Attention: {{attention}}%</span></div>
        <div style="font-size:.64em;color:#6b7280">
          Face: {{'✅' if face else '❌'}} | Fingers: {{fingers}} | Streak: {{streak}}</div>
      </div>
    </div>
    <div class="card"><h2>📊 Skills Progress</h2>
      <div class="skill-grid">
        <div class="skill"><div class="skill-val">{{skill_motor}}%</div><div style="font-size:.7em;color:#6b7280">Motor</div></div>
        <div class="skill"><div class="skill-val">{{skill_cog}}%</div><div style="font-size:.7em;color:#6b7280">Cognitive</div></div>
        <div class="skill"><div class="skill-val">{{skill_verbal}}%</div><div style="font-size:.7em;color:#6b7280">Verbal</div></div>
        <div class="skill"><div class="skill-val">{{skill_math}}%</div><div style="font-size:.7em;color:#6b7280">Math</div></div>
      </div>
    </div>
    <div class="card"><h2>📝 Parent Notes</h2>
      ...
      <div class="note">{{n.time}}: {{n.text}}</div>
      {% endfor %}
    </div>
  </div>
  <div>
    ...
    <div class="card"><h2>🏆 Session Summary</h2>
      <div style="font-size:.75em;line-height:1.8;color:#9ca3af">
        ...
        Level: {{level}} | Domain: {{domain}}<br>
        OK: <span style="color:#34d399">{{ok}}</span> | 
        Fail: <span style="color:#f87171">{{fail}}</span> | 
        Skipped: <span style="color:#f97316">{{skipped}}</span><br>
...
def parent_home():
    return render_template_string(PARENT_HTML,
        ...
        attention=ST["attention"],skipped=ST["tasks_skipped"],
        emotion=ST["emotion"],face=ST["face_detected"],
        fingers=ST["finger_count"],streak=ST["streak"],
        skill_motor=ST["skill_motor"],skill_cog=ST["skill_cognitive"],
        skill_verbal=ST["skill_verbal"],skill_math=ST["skill_math"],
        notes=ST["parent_notes"],chat=ST["session_chat"],
        logs=ST["logs"],level=ST["current_level"],
        domain=ST["domain"],ok=ST["tasks_success"],
        fail=ST["tasks_fail"],date=ST["session_date"],csv=CSV_FILE,
        local_ip=LOCAL_IP)
...
def parent_state():
    return jsonify({
        "name":CHILD_NAME,"emotion":ST["emotion"],"attention":ST["attention"],
        "score":ST["score"],"mastered":ST["tasks_mastered"],
        "finger_count":ST["finger_count"],"is_speaking":ST["is_speaking"],
...
.b1{background:#4f46e5;color:#fff}.b2{background:#059669;color:#fff}
.b3{background:#1d4ed8;color:#fff}
.card{background:#0c0f1e;border-radius:10px;padding:12px;border:1px solid #1a1f40;margin-bottom:8px}
.card h2{color:#818cf8;margin-bottom:6px;font-size:.84em}
...
tr:hover td{background:#0c0f1e}
.sk{display:inline-block;padding:2px 8px;border-radius:10px;font-size:.7em}
.s-ok{background:#052918;color:#34d399;border:1px solid #34d399}
.s-fail{background:#45090a;color:#f87171;border:1px solid #f87171}
.s-skip{background:#2a1200;color:#f97316;border:1px solid #f97316}
</style></head><body>
...
<div class="card"><h2>📊 Session Statistics</h2>
  ...
  <div class="stat"><div class="n">{{attention}}%</div><div class="l">Attention</div></div>
  <div class="stat"><div class="n">{{level}}</div><div class="l">Level</div></div>
</div>
<div class="card"><h2>🎯 Skills Profile</h2>
  <table><tr><th>Skill</th><th>Level</th><th>Trend</th></tr>
  <tr><td>Motor (ABA)</td><td>{{skill_motor}}%</td>
    <td style="color:{{'#34d399' if skill_motor>60 else '#f87171'}}">
      {{'Good ✅' if skill_motor>60 else 'Needs work ⚠️'}}</td></tr>
  <tr><td>Cognitive (TEACCH)</td><td>{{skill_cog}}%</td>
    <td style="color:{{'#34d399' if skill_cog>60 else '#f87171'}}">
      {{'Good ✅' if skill_cog>60 else 'Needs work ⚠️'}}</td></tr>
  <tr><td>Verbal (DTT)</td><td>{{skill_verbal}}%</td>
    <td style="color:{{'#34d399' if skill_verbal>60 else '#f87171'}}">
      {{'Good ✅' if skill_verbal>60 else 'Needs work ⚠️'}}</td></tr>
  <tr><td>Math (Counting)</td><td>{{skill_math}}%</td>
    <td style="color:{{'#34d399' if skill_math>60 else '#f87171'}}">
      {{'Good ✅' if skill_math>60 else 'Needs work ⚠️'}}</td></tr>
  </table>
</div>
<div class="card"><h2>📝 Session Log (CSV: {{csv}})</h2>
  <table><tr><th>Time</th><th>Task</th><th>Domain</th><th>Result</th><th>Emotion</th></tr>
  {% for lg in logs[-40:]|reverse %}
  <tr>
    <td>{{lg.time}}</td>
    <td>{{lg.msg[:40]}}</td>
    <td>{{lg.get('proto','—')}}</td>
    <td><span class="sk {{'s-ok' if lg.type=='success' else 's-fail' if lg.type=='fail' else ''}}">
      {{lg.type.upper()}}</span></td>
    <td>{{lg.emo}}</td>
  </tr>{% endfor %}
  </table>
</div>
<div class="card"><h2>👨‍👩‍👦 Parent Notes</h2>
  ...
  {% if not notes %}<p style="color:#6b7280;font-size:.75em">No notes yet — add them in the dashboard.</p>{% endif %}
</div>
<div class="card"><h2>🌐 Android Access</h2>
  <p style="color:#34d399;font-size:.8em">Same WiFi → browser → http://{{local_ip}}:5007</p>
</div>
</body></html>"""
...
def report_home():
    return render_template_string(REPORT_HTML,
        ...
        skipped=ST["tasks_skipped"],streak=ST["streak"],
        attention=ST["attention"],level=ST["current_level"],
        skill_motor=ST["skill_motor"],skill_cog=ST["skill_cognitive"],
        skill_verbal=ST["skill_verbal"],skill_math=ST["skill_math"],
        logs=ST["logs"],notes=ST["parent_notes"],
        csv=CSV_FILE,local_ip=LOCAL_IP)
...
.nav a{padding:7px 13px;border-radius:8px;text-decoration:none;font-size:.78em;font-weight:700}
.b1{background:#4f46e5;color:#fff}.b2{background:#059669;color:#fff}.b3{background:#1d4ed8;color:#fff}
.sc{background:#1a0a3d;border-radius:8px;padding:7px 14px;text-align:center;margin-bottom:8px;color:#a78bfa}
...
.gc{background:#0c0f1e;border:2px solid #1a1f40;border-radius:12px;padding:14px;
    text-align:center;cursor:pointer;transition:.25s}
.gc:hover,.gc:active{border-color:#a78bfa;transform:translateY(-2px)}
.gi{font-size:2.2em;margin-bottom:5px}
...
.btn{background:#4f46e5;color:#fff;border:none;padding:9px 16px;
     border-radius:7px;cursor:pointer;font-size:.82em;margin:4px;
...
function start(g){var d=document.getElementById('ag');
  ...
  else if(g==='animals')animalGame(d);else if(g==='fruits')fruitGame(d);
  else wordGame(d);}
function balloons(d){var W=Math.min(760,window.innerWidth-24);
  ...
  for(var i=0;i<12;i++)bs.push({x:Math.random()*(W-40)+20,y:Math.random()*260+20,
    r:16+Math.random()*12,vx:(Math.random()-0.5)*2.5,vy:(Math.random()-0.5)*2.5,
  ...
  function hit(mx,my){bs.forEach(function(b){if(!b.alive)return;
    ...
    if(bs.every(function(b){return !b.alive;}))bs.forEach(function(b){b.alive=true;
      b.x=Math.random()*(W-40)+20;b.y=Math.random()*260+20;});}
...
function emoGame(d){var em=[['😊','Happy'],['😢','Sad'],['😠','Angry'],['😨','Scared'],['😲','Surprised'],['😄','Joyful']];
  ...
  d.innerHTML='<div style="text-align:center;padding:16px">'+
    ...
    ' style="display:block;width:100%;max-width:220px;margin:12px auto">I did it! 🌟</button>'+
    '<button class="btn" onclick="emoGame(document.getElementById(\'ag\'))"'+
    ' style="display:block;width:100%;max-width:220px;margin:6px auto">Next ➡️</button></div>';}
function colorGame(d){var cs=[['Red','#ef4444'],['Blue','#3b82f6'],['Green','#22c55e'],
  ...
  d.innerHTML='<div style="text-align:center;padding:14px">'+
    ...
    '<div style="width:100px;height:100px;background:'+cs[idx][1]+
...
window.chkC=function(ch,co){var el=document.getElementById('cbtns');
  if(ch===co){add(15);el.innerHTML='<p style="color:#34d399;margin:8px;font-size:1.1em">✅ Correct! 🌟</p>'+
    '<button class="btn" onclick="colorGame(document.getElementById(\'ag\'))" style="display:block;margin:6px auto">Next ➡️</button>';}
  else el.innerHTML='<p style="color:#f87171;margin:8px">Try again! 💪</p>'+
    '<button class="btn" onclick="colorGame(document.getElementById(\'ag\'))" style="display:block;margin:6px auto">Retry ↩️</button>';};
function numGame(d){var n=Math.floor(Math.random()*10)+1,stars='';
  ...
  d.innerHTML='<div style="text-align:center;padding:14px">'+
    '<p style="color:#a78bfa;font-size:1.1em">Count the stars!</p>'+
    ...
    opts.map(function(x){return '<button class="btn" onclick="chkN('+x+','+n+')" '+
      'style="font-size:1.2em;min-width:60px;padding:10px">'+x+'</button>';}).join('')+'</div></div>';}
window.chkN=function(ch,co){if(ch===co){add(20);alert('✅ YES! '+co+'! 🌟');
  numGame(document.getElementById('ag'));}else alert('Try again! 💪');};
function memGame(d){var pairs=['🐶','🐱','🐻','🦊','🐼','🐨','🦁','🐯'];
  ...
  d.innerHTML='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;max-width:360px;margin:10px auto">'+
    cards.map(function(c,i){return '<div id="mc'+i+'" onclick="flipM('+i+')" '+
      'style="height:70px;background:#1f2937;border-radius:9px;display:flex;align-items:center;'+
      'justify-content:center;font-size:2em;cursor:pointer;border:2px solid #374151;touch-action:manipulation">❓</div>';
    }).join('')+'</div>';}
window.flipM=function(idx){var c=window._mc;
  ...
  if(window._mf.length===2){var a=window._mf[0],b=window._mf[1];
    if(c[a]===c[b]){window._mm.push(a,b);add(30);window._mf=[];
      if(window._mm.length===c.length)setTimeout(function(){alert('🎉 ALL MATCHED!');
        memGame(document.getElementById('ag'));},400);}
    else setTimeout(function(){document.getElementById('mc'+a).textContent='❓';
      document.getElementById('mc'+b).textContent='❓';window._mf=[];},900);}};
function shapeGame(d){var shapes=[['⬛','Square'],['⭕','Circle'],['🔺','Triangle'],
    ['💎','Diamond'],['⭐','Star'],['❤️','Heart'],['🌙','Moon']];
  var pick=shapes[Math.floor(Math.random()*shapes.length)];
  d.innerHTML='<div style="text-align:center;padding:14px">'+
    '<p style="color:#a78bfa;margin-bottom:8px;font-size:1.1em">What shape is this?</p>'+
    '<div style="font-size:4.5em;margin:10px">'+pick[0]+'</div>'+
    '<div style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px">'+
    shapes.map(function(s){return '<button class="btn" onclick="chkS(\''+
      s[1]+'\',\''+pick[1]+'\')" style="min-width:80px">'+s[1]+'</button>';}).join('')+'</div></div>';}
window.chkS=function(ch,co){if(ch===co){add(15);alert('✅ Correct! 🌟');
  shapeGame(document.getElementById('ag'));}else alert('Try again! 💪');};
function animalGame(d){var animals=[['🐶','Dog'],['🐱','Cat'],['🦁','Lion'],['🐘','Elephant'],
    ['🐰','Rabbit'],['🐻','Bear'],['🐯','Tiger'],['🐧','Penguin'],['🦒','Giraffe']];
  var pick=animals[Math.floor(Math.random()*animals.length)];
  d.innerHTML='<div style="text-align:center;padding:14px">'+
    '<p style="color:#a78bfa;margin-bottom:8px;font-size:1.1em">Which animal is this?</p>'+
    '<div style="font-size:5em;margin:10px">'+pick[0]+'</div>'+
    '<div style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px">'+
    animals.slice(0,5).concat([pick]).sort(function(){return Math.random()-0.5;}).slice(0,6)
      .map(function(a){return '<button class="btn" onclick="chkA(\''+
        a[1]+'\',\''+pick[1]+'\')" style="min-width:80px">'+a[1]+'</button>';}).join('')+'</div></div>';}
window.chkA=function(ch,co){if(ch===co){add(15);alert('✅ Correct! 🌟');
  animalGame(document.getElementById('ag'));}else alert('Try again! 💪');};
function fruitGame(d){var fruits=[['🍎','Apple'],['🍌','Banana'],['🍊','Orange'],
    ['🍇','Grapes'],['🍓','Strawberry'],['🍉','Watermelon'],['🥭','Mango'],['🍍','Pineapple']];
  var pick=fruits[Math.floor(Math.random()*fruits.length)];
  d.innerHTML='<div style="text-align:center;padding:14px">'+
    '<p style="color:#a78bfa;margin-bottom:8px;font-size:1.1em">Which fruit is this?</p>'+
    '<div style="font-size:5em;margin:10px">'+pick[0]+'</div>'+
    '<div style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px">'+
    fruits.slice(0,5).concat([pick]).sort(function(){return Math.random()-0.5;}).slice(0,6)
      .map(function(a){return '<button class="btn" onclick="chkF(\''+
        a[1]+'\',\''+pick[1]+'\')" style="min-width:80px">'+a[1]+'</button>';}).join('')+'</div></div>';}
window.chkF=function(ch,co){if(ch===co){add(15);alert('✅ Correct! 🌟');
  fruitGame(document.getElementById('ag'));}else alert('Try again! 💪');};
function wordGame(d){var words=['APPLE','BALL','CAT','DOG','ELEPHANT','FISH','GOOD',
    'HAPPY','JUMP','KITE','LOVE','MILK','PLAY','RED','SUN','TREE','WATER'];
  var word=words[Math.floor(Math.random()*words.length)];
  d.innerHTML='<div style="text-align:center;padding:16px">'+
    '<p style="color:#a78bfa;margin-bottom:8px;font-size:1.1em">Can you say this word?</p>'+
    '<div style="font-size:2.5em;font-weight:700;color:#a78bfa;margin:12px;'+
    'background:#1e1b4b;padding:15px;border-radius:15px">'+word+'</div>'+
    '<button class="btn btn-g" onclick="add(20);this.textContent=\'✅ Said it!\'"'+
    ' style="display:block;width:100%;max-width:200px;margin:10px auto">I said it! 🎤</button>'+
    '<button class="btn" onclick="wordGame(document.getElementById(\'ag\'))"'+
    ' style="display:block;width:100%;max-width:200px;margin:6px auto">Next word ➡️</button></div>';}
</script></body></html>"""
...
class TherapyCtrl:
    """
    CLINICAL LOGIC:
    - ABA: Discrete trials, reinforcement
    - DTT: SD → Response → Consequence; 2 fails → skip + report
    - TEACCH: Visual structure, locked UI until response
    - ESDM: Social joy, gaze following, joint attention
    """
    def __init__(self,voice:Voice,sim:PepperSim):
        self.v=voice; self.s=sim; self.running=False
    ...
    def run(self):
        ...
        self._say(
            ...
            "Let us earn 3 stars for each task! Ready?")
        time.sleep(0.5)
        ...
        while self.running:
            ...
            ST["fail_count"]=0  # Reset fail counter per task attempt cycle

            # TEACCH: Show task on tablet
            self._show_tablet(task)

            # ESDM: Gaze to tablet for joint attention
            ...
            if success:
                ...
                self._say(
                    ...
                    "Now the NEXT task! Great job! [CELEBRATE]",
                    wait=False)
            else:
                # DTT: 2 fails — generate mini-report + skip
                self._on_skip(task)
            time.sleep(0.3)
    ...
    def _run_task_dtt(self,task)->bool:
        """
        DTT protocol: Stimulus → Response → Consequence
        Max MAX_FAILS wrong before skip + mini-report
        """
        ...
        deadline=time.time()+60; last_p=time.time(); pidx=0
        fail_count=0
        ...
        while time.time()<deadline:
            ...
            if time.time()-last_p>6:
                last_p=time.time()
                self.v.say(f"{CHILD_NAME}... {prompts[pidx%len(prompts)]}",wait=False)
                BRIDGE.sig_set_waiting.emit(waiting); pidx+=1

            self._handle_cmd()
            time.sleep(0.18)

        ST["verify_action"]=None
        ST["fail_count"]=fail_count; return False
    ...
    def _on_success(self,task):
        ...
        ST["att_history"].append(ST["attention"]); ST["score_history"].append(ST["score"])
        if len(ST["att_history"])>60: ST["att_history"]=ST["att_history"][-60:]
        ...
        # Chat notification
        BRIDGE.sig_chat_msg.emit("pepper",
            f"⭐ Well done {CHILD_NAME}! {task.get('success','Great job!')} Score: {ST['score']}")
        ...
        if ST["consecutive"]>=MASTERY_N:
            ...
            self._say(
                ...
                f"Moving to next task: {ntask.get('name','')}! [LEVEL_UP]",
                wait=False)
            BRIDGE.sig_chat_msg.emit("pepper",
                f"🏆 LEVEL UP! {CHILD_NAME} mastered: {task.get('name',task['id'])}!")
            QTimer.singleShot(4000,lambda: ST.update(
                {"social_joy_active":False,"eye_color":(100,180,255)}))

    def _on_skip(self,task):
        """DTT: 2 fails → mini clinical report → next task"""
        ST["tasks_fail"]+=1; ST["tasks_skipped"]+=1
        ST["streak"]=0; ST["consecutive"]=0
        ST["task_index"]=(ST["task_index"]+1)%len(TASK_POOL)

        # Mini-report in chat
        report=f"📋 Task '{task.get('name',task['id'])}' skipped after {MAX_FAILS} attempts. "
        report+=f"Domain: {task['domain']}. Skill: {task.get('verify','')}. "
        report+="Recommendation: More practice needed for this skill."
        BRIDGE.sig_chat_msg.emit("pepper",report)
        BRIDGE.sig_show_skip.emit(f"Skipping after {MAX_FAILS} tries. Moving on!")
        ST["parent_notes"].append({"time":datetime.now().strftime("%H:%M"),
            "text":f"AUTO: Skipped '{task.get('name',task['id'])}' after {MAX_FAILS} fails — needs attention"})

        self._say(
            f"{CHILD_NAME}, good try! Let us try something different! "
            f"We will practice {task.get('name',task['id'])} again soon!",
            wait=False)
    ...
    def _handle_cmd(self):
        ...
        elif cmd=="report": self._print_report()

    def _print_report(self):
        dur=int((time.time()-ST["uptime"])/60)
        print(f"\n{'='*55}")
        print(f"CLINICAL REPORT — {CHILD_NAME} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
        print(f"Duration: {dur} min | Score: {ST['score']} | Level: {ST['current_level']}")
        print(f"Mastered: {ST['tasks_mastered']} | OK: {ST['tasks_success']} | Fail: {ST['tasks_fail']} | Skipped: {ST['tasks_skipped']}")
        print(f"Motor:{ST['skill_motor']}% Cognitive:{ST['skill_cognitive']}% Verbal:{ST['skill_verbal']}% Math:{ST['skill_math']}%")
...
def main():
    print(f"""
╔══════════════════════════════════════════════════════════╗
║  PEPPER CLINICAL INFINITY V2 — {CHILD_NAME:<24}║
╠══════════════════════════════════════════════════════════╣
║  FIXES: Emotion✅ | Avatar✅ | Mic✅ | Chat✅          ║
║  DTT: 2 fails → skip + report | Endless tasks           ║
║  ABA/DTT/TEACCH/ESDM | {len(TASK_POOL)} task variations          ║
╚══════════════════════════════════════════════════════════╝
""")

    # Flask servers
    for app,port,name in [(parent_app,5007,"Parent Dashboard"),
    ...
    qt_app.setApplicationName(f"Pepper Clinical Infinity V2 — {CHILD_NAME}")
    ...
    tablet=TabletWindow(); tablet.show(); tablet.move(20,560)

    cam_thread=CameraThread(); cam_thread.start()
    em_win=EmotionWindow()        # W2: 300×300
    live_win=LiveWindow(cam_thread) # W3: resizable

    sim=PepperSim(); sim.launch()
    voice=Voice()
    ctrl=TherapyCtrl(voice,sim)
    ...
    print(f"""
{'='*60}
✅ CLINICAL INFINITY V2 ACTIVE
{'='*60}
W1: PyBullet    (FIXED — lip-sync + micro-anims)
W2: Emotion     (300×300 — FIXED emotion detection)
W3: Live        (Resizable — avatar + body tracking)
W4: Tablet      (FIXED 1300×800 — camera + chat + tasks)
...
""")
        ...
        while ctrl.running:
            try:
                ...
                if cmd in ["q","exit","quit"]:
                    ...
                    em_win.stop(); live_win.stop(); qt_app.quit(); break
    ...
    ret=qt_app.exec()
    ctrl.running=False; cam_thread.stop(); em_win.stop(); live_win.stop()
    ...
    print(f"Score:{ST['score']} | Mastered:{ST['tasks_mastered']} | Skipped:{ST['tasks_skipped']}")
    print(f"CSV: {CSV_FILE} | JSON: {fn}")
    print('='*60)
    sys.exit(ret)
