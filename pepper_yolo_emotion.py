#!/usr/bin/env python3
"""
Pepper with YOLO Object Detection + Emotion Recognition
- Detects people in camera feed
- Detects emotions (Happy, Sad, Neutral, Surprise)
- Shows results in terminal
- Opens ASD games at localhost:5000
...
class VisionDetector:
    ...
    def start_camera(self):
        try:
            ...
            if not self.camera.isOpened():
                print("⚠️ No camera found! Using fallback mode.")
                self.camera = None
            else:
                print("✅ Camera started!")
                threading.Thread(target=self.detect_loop, daemon=True).start()
        except:
            print("⚠️ Camera error! Running without vision.")
            self.camera = None
    ...
    def detect_loop(self):
        while self.running:
            ...
            # Print to terminal
            if persons:
                emotion = self.last_detection.get("emotion", "unknown")
                print(f"\r👤 Person detected! Emotion: {emotion.upper()} {'😊' if emotion=='happy' else '😐' if emotion=='neutral' else '😢' if emotion=='sad' else '😲'}", end="")
            else:
                print(f"\r👀 No person detected", end="")
            
            # Draw on frame (optional - can be disabled)
    ...
    def get_current_emotion(self):
        if time.time() - self.last_detection.get("time", 0) < 2:
            return self.last_detection.get("emotion", "neutral")
        return "none"
...
class PepperRobot:
    def __init__(self):
        ...
        self.show_text("Hello! I'm Pepper! I see you! 👀")
        
        print("\n" + "="*55)
        print("🤖 PEPPER with YOLO + Emotion Detection")
        print("="*55)
        print("🎈 Pepper chases balloons")
        print("📷 Camera detects: Person + Emotion")
        print("📝 Commands:")
        print("   game - Opens ASD games (localhost:5000)")
    ...
    def open_game(self):
        ...
        return "🎮 Opening your ASD games at localhost:5000! Have fun playing! 🎲"
    ...
    def run(self):
        while True:
            try:
                # Show current emotion in prompt
