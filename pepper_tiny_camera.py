#!/usr/bin/env python3
"""
Pepper with TINY Camera Window (250x200) + Games on localhost:5001
"""
...
# ========== YOLO + Emotion with TINY Camera Window ==========
class VisionDetector:
    ...
    def start_camera(self):
        try:
            ...
            if self.camera.isOpened():
                print("✅ Camera ready! TINY window (250x200)...")
    ...
    def detect_loop(self):
        while self.running and self.camera:
            ...
            # Make frame VERY SMALL (250x200)
            frame = cv2.resize(frame, (250, 200))
            
            # YOLO detection (faster on small frame)
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        if self.yolo.names[cls] == 'person':
                            ...
                            if face.size > 0 and face.shape[0] > 15 and face.shape[1] > 15:
                                try:
                            ...
                            cv2.putText(frame, f"{PERSON_NAME}", (x1, y1-12), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
                            cv2.putText(frame, f"{self.current_emotion[:3]}", (x1, y1-2), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 0), 1)
            
            # Terminal output
            if self.person_detected:
                print(f"\r👤 {PERSON_NAME} | {self.current_emotion.upper()}", end="")
            else:
                print(f"\r👀 No person", end="")
            
            # Show TINY camera window
            cv2.imshow('Pepper Vision', frame)
            cv2.resizeWindow('Pepper Vision', 250, 200)
...
class PepperAI:
    ...
    def chat(self, message):
        ...
        data = {
            "model": "openai",
            "messages": [
                {"role": "system", "content": "You are Pepper. Respond in 1 short, fun sentence in ENGLISH."},
...
class PepperRobot:
    def __init__(self):
        ...
        for i in range(6):
            self.balloons.append(Balloon(random.uniform(-3,3), random.uniform(-2.2,2.2), colors[i%4]))
        ...
        print("\n" + "="*55)
        print("🤖 PEPPER - TINY Camera | Games: localhost:5001")
        print("="*55)
        print("📝 Commands: game | how to... | picture... | dance | wave")
        print("="*55)
    ...
    def open_game(self):
        ...
        return f"🎮 Opening your games at localhost:5001! 🎲"
        ...
        while True:
            try:
                ...
                if result:
                    ...
                    self.show_text(self.ai.chat(user))
